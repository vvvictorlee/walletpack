...
export const roxejsUtil = getRoxejsApi();

const MAINNET_CHAIN_ID = '1a7c1e220c79277068a08fe57bb0461ed65ccc39950324272f63cfaeb7db10c7';
...
export default class ROXE extends Plugin {

	constructor(){ super(Blockchains.ROXE, PluginTypes.BLOCKCHAIN_SUPPORT) }
	...
	getSignableRoxejs(accounts, reject, prompt = true, signatureProvider = null){
		const isSingleAccount = accounts instanceof Account;
	...
	returnableAccount(account){ return { name:account.name, authority:account.authority, publicKey:account.publicKey, blockchain:Blockchains.ROXE }}

	contractPlaceholder(){ return 'roxe.token'; }
	...
	async proxyVote(account, proxyAccount, prompt = false){
		return new Promise(async (resolve, reject) => {
			...
			const eos = this.getSignableRoxejs(account, reject, prompt);
	...
	async changePermissions(account, keys){
		if(!keys) return;
		return new Promise(async (resolve, reject) => {
			const eos = this.getSignableRoxejs(account, reject);
			...
			return eos.transact({actions},{
				...
				.then(async res => {
					...
					const addAccount = async (keypair, authority) => {
						...
						acc.publicKey = keypair.publicKeys.find(x => x.blockchain === Blockchains.ROXE).key,
	...
	getImportableAccounts(keypair, network){
		return new Promise((resolve, reject) => {
			let publicKey = keypair.publicKeys.find(x => x.blockchain === Blockchains.ROXE);
	...
	privateToPublic(privateKey, prefix = null){ try {
		return ecc.PrivateKey(privateKey).toPublic().toString(prefix ? prefix : Blockchains.ROXE.toUpperCase());
	...
	validPublicKey(publicKey, prefix = null){
		try {
			return ecc.PublicKey.fromStringOrThrow(publicKey, prefix ? prefix : Blockchains.ROXE.toUpperCase());
		} catch(e){
			return false;
		}
	}
	...
	async balancesFor(account, tokens, fallback = false){
		if(!fallback && await LightAPI.networkString(account.network())){
			...
			const blacklist = StoreService.get().state.scatter.settings.blacklistTokens.filter(x => x.blockchain === Blockchains.ROXE).map(x => x.unique());
	...
	defaultToken(){ return new Token(Blockchains.ROXE, 'roxe.token', 'ROXE', 'ROXE', this.defaultDecimals(), MAINNET_CHAIN_ID) }
	...
	async transfer({account, to, amount, token, memo, promptForSignature = true}){
		...
		return new Promise(async (resolve, reject) => {
			const eos = this.getSignableRoxejs(account, reject, promptForSignature);
	...
	async parseRoxejsRequest(payload, network){
		try {
			const {transaction} = payload;
			const api = getRoxejsApi();
	...
	async parseRoxejs2Request(payload, network){
		const {transaction} = payload;
	...
	async requestParser(payload, network){
		if(payload.transaction.hasOwnProperty('serializedTransaction'))
			return this.parseRoxejs2Request(payload, network);
		else return this.parseRoxejsRequest(payload, network);
	}
}
